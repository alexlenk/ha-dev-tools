...
class ValidationManager:
    ...
    def validate_content(self, content: str, file_path: str) -> ValidationResult:
        ...
        if file_extension in [".yaml", ".yml"]:
            ...
            return ValidationResult(
                is_valid=True, errors=[], warnings=[], line_numbers=[]
            )
            ...
            if file_path == "configuration.yaml":
                if not isinstance(parsed_data, dict):
                    ...
                    if "homeassistant" not in parsed_data:
                        warnings.append(
                            "Missing 'homeassistant' section in configuration"
                        )
    ...
    def validate_json(self, content: str, file_path: str) -> ValidationResult:
        ...
        try:
            # Only syntax validity matters here - the parsed value itself
            # isn't returned, just whether json.loads() raises.
            json.loads(content)
    ...
    def validate_storage_file(
        self, content: str, storage_type: str
    ) -> ValidationResult:
        """
