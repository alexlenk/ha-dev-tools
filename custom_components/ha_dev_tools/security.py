...
class SecurityManager:
    ...
    def _build_allowlist(self) -> set[str]:
        ...
        Merges read_paths and write_paths into a unified allowlist. Given
        _build_read_paths()/_build_write_paths()'s reciprocal defaulting
        rule, at least one of them always has real entries (proved: for
        both to end up empty, each would have needed the other to be
        explicitly non-empty at the raw config level, which is
        self-contradictory) - so the historical "allowlist ended up empty,
        fall back to something" cases below can no longer happen and were
        removed along with the allowed_paths/allowed_storage_files legacy
        config keys that used to feed them (see CHANGELOG).
    ...
    def _validate_configured_paths(self) -> None:
        """
        Validate configured denied_paths for security issues.

        Logs warnings for denied_paths entries that use path traversal (..)
        or fall outside /config/ or /addon_configs/.
        """
    ...
    def _is_valid_path_config(self, path: str) -> bool:
        """
        Check if a configured path is valid: no traversal, must start with
        /config/ or /addon_configs/.
    ...
    def _log_security_config(self) -> None:
        ...
        if not configured_read and not configured_write:
            _LOGGER.info(
                "Using default read-only paths: %d paths", len(DEFAULT_READ_ONLY_PATHS)
            )
    ...
    def _matches_glob_pattern(self, path: str, pattern: str) -> bool:
        ...
        """
        return self._fnmatch_globstar(path, pattern) or (
    ...
    def validate_file_path(
        self, file_path: str, operation: str = OPERATION_READ
        ...
        try:
            ...
            if self.is_denylisted(normalized_path) or self.is_denylisted(
                normalized_path_with_prefix
            ):
                self.log_security_event(
            ...
            if operation == OPERATION_WRITE:
                ...
                # Check both with and without /config/ prefix
                if not (
                    self.is_writable(normalized_path)
                    or self.is_writable(normalized_path_with_prefix)
                ):
                    self.log_security_event(
                        "write_access_denied",
                        {"path": file_path, "reason": "path_not_in_write_paths"},
                    )
                ...
                # Check both with and without /config/ prefix
                if not (
                    self.is_allowlisted(normalized_path)
                    or self.is_allowlisted(normalized_path_with_prefix)
                ):
                    self.log_security_event(
            ...
            is_explicitly_allowlisted = (
                normalized_path in self.allowlist
                or normalized_path_with_prefix in self.allowlist
            )

            if (
                not is_explicitly_allowlisted
                and not normalized_path.startswith(".storage/")
                and not normalized_path.startswith("/config/.storage/")
            ):
    ...
    def is_denylisted(self, file_path: str) -> bool:
        ...
        for denied in self.denylist:
            ...
            if normalized_path.startswith(denied + "/") or normalized_path.startswith(
                denied + os.sep
            ):
                return True
            ...
            if "*" in denied or "?" in denied:
                ...
                # allows ".storage/onboarding" to match "/config/.storage/onboarding"
                if self._matches_glob_pattern(normalized_path, denied):
                    return True
    ...
    def is_allowlisted(self, file_path: str) -> bool:
        ...
        for allowed in self.allowlist:
            ...
            if normalized_path.startswith(allowed + "/") or normalized_path.startswith(
                allowed + os.sep
            ):
                return True
    ...
    def validate_user_permissions(
        ...
        if not user.is_admin:
            self.log_security_event(
                "non_admin_access_attempt", {"user_id": user.id, "user_name": user.name}
            )
            return False, ERROR_PERMISSION_DENIED
    ...
    def log_security_event(self, event_type: str, details: dict[str, Any]) -> None:
        ...
        """
        _LOGGER.warning("Security event: %s - %s", event_type, details)
    ...
    def remove_from_blacklist(self, file_path: str) -> None:
        ...
        Deprecated: use remove_from_denylist() instead - kept for backward
        compatibility.
    ...
    def add_to_denylist(self, file_path: str) -> None:
        ...
        if not self._is_valid_path_config(file_path):
            _LOGGER.warning(
                "Cannot add invalid path to denylist: %s (path traversal or absolute path)",
                file_path,
            )
            return
        ...
        self.blacklist.add(
            file_path
        )  # Keep blacklist in sync for backward compatibility
    ...
    def add_to_allowlist(self, file_path: str) -> None:
        ...
        if not self._is_valid_path_config(file_path):
            _LOGGER.warning(
                "Cannot add invalid path to allowlist: %s (path traversal or absolute path)",
                file_path,
            )
            return
