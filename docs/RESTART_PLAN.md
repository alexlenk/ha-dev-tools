...
4. **Configure surface (helpers/dashboards)** — ❌ blocked, not started: `input_boolean`/`counter`/`timer`/etc.'s `StorageCollection` is a private local variable inside each component's own `async_setup`, never exposed via `hass.data` - confirmed by reading the actual home-assistant/core source. The only real interface is the WebSocket API the frontend uses; calling it correctly from in-process code needs a proper loopback WS-client call, which is real, version-sensitive work that deserves its own dedicated pass rather than being rushed rather than guessed at with something that risks desyncing helper state.
5. **Audit/hygiene** — ⚠️ partial: `audit_automations` (duplicate ids + unavailable-entity references) landed. Overlapping-trigger race detection and unhandled rest_command/shell_command failure detection are real but need more careful semantic analysis to avoid false positives - deliberately deferred rather than shipped unreliable. `entity_health_report` and `bulk_update_entities` (Concrete tool list) not started.
