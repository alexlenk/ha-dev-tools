...
2. **Core authoring loop** — ✅ landed (`feature/llm-tools-foundation`): layout detection, `get_logs` (raw file), `find_entities`, `get_automation`/`write_automation`, `check_config`/`reload_domain`, `render_template`/`validate_template` (in-process via `homeassistant.helpers.template.Template`, no WS/HTTP round-trip needed - `template_manager.py`).
...
6. **Monitor** — ✅ landed: `list_addons`/`get_addon_logs` (`supervisor_manager.py`), gated behind a `SupervisorNotAvailableError` guard for Core-only installs (most real installs, including this test environment). Logs go through the lower-level `HassIO.send_command()` REST wrapper (`/addons/{slug}/logs`) rather than the typed `aiohasupervisor` client, since that client's `addons` object has no `logs` method in the installed HA version. Can't be verified end-to-end here - no way to stand up a real Supervisor in a sandbox, only under actual HA OS/Supervised; tests cover the guard and response-shaping logic against mocks.
...
8. **Access control** — ✅ landed (`access_control.py`, `GatedTool` in `llm_api.py`): every tool except `dev_tools_ping` now requires an out-of-band "arm file" (only real filesystem access can create it - SSH, Terminal add-on, never dev_tools itself) plus a genuine HA admin check, independent of `mcp_server`'s own gate. This exists because HA's token model has no scoping (any of a user's tokens is as powerful as any other, confirmed by reading `auth/models.py`) and `mcp_server`'s admin check only covers the explicit `/api/mcp/<api_id>` URL, not the bare `/api/mcp` endpoint - without this, dev_tools' file/shell-adjacent capability would be reachable by any leaked ordinary HA credential. See the CHANGELOG's Security entry for the full reasoning.

**Also open, not yet in any tool**: blueprint resolution for `get_automation`/`audit_automations`.

**Corrected: the "CI can't validate the mcp_server path" claim above was wrong.** Earlier iterations of this doc said `homeassistant==2025.1.4` was "the latest release actually published to PyPI" and that `mcp_server` "hadn't reached a stable release yet." Neither is true - `homeassistant==2026.8.2` (with `mcp_server`, confirmed by reading its manifest at that exact git tag) has been on PyPI since 2026-08-14. What was actually happening: HA's own `Requires-Python` floor climbed to `>=3.14.2` starting with the 2026.3 release line (checked directly against PyPI release metadata, not assumed), and CI's test matrix only ran Python 3.12/3.13. `pip install homeassistant>=2024.1.0` under those interpreters can't even see 2026.8.2 - it silently falls back to 2025.1.4, the newest release still supporting Python 3.12, which looked identical to "not released yet" from CI's output but wasn't. Fixed: `test.yml`'s matrix now runs Python 3.14, and `requirements-test.txt` pins `pytest-homeassistant-custom-component==0.13.356` (whose own exact pin is `homeassistant==2026.8.2`) instead of a bare floor. This should let CI genuinely validate tool registration against the real `mcp_server`-having release - still not the actual wire protocol (`/api/mcp/dev_tools` over Streamable HTTP, which needs a running hass instance and a real MCP client, not just an importable package), but a large step closer than before. Couldn't be verified locally in this sandbox before pushing - the sandbox's only local Python 3.14 build (via `uv`) is `3.14.0rc2`, which doesn't satisfy HA's `>=3.14.2` floor either - so this is verified against real CI output, per this project's established practice of using GitHub Actions as the source of truth over local runs.
