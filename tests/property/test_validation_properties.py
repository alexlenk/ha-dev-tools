...
valid_yaml_content = st.one_of(
    [
        ...
        st.builds(lambda: """homeassistant:
  name: Test Home
  latitude: 32.87336
  longitude: -117.22743
"""),
...
valid_json_content = st.builds(
    ...
    value=st.one_of(
        st.text(
            alphabet=string.ascii_letters + string.digits + " ", min_size=0, max_size=50
        ),
        st.integers(min_value=-1000, max_value=1000),
        st.floats(
            allow_nan=False, allow_infinity=False, min_value=-1000, max_value=1000
...
def test_invalid_yaml_always_fails_validation(yaml_content):
    ...
    **Validates: Requirements 3.1, 3.3**
    ...
    assert result.is_valid is False, "Invalid YAML passed validation"
    assert len(result.errors) > 0, "Invalid YAML has no error messages"
    assert (
        "YAML syntax error" in result.errors[0]
    ), "Error message doesn't mention YAML syntax"
...
def test_invalid_json_always_fails_validation(json_content):
    ...
    **Validates: Requirements 3.2, 3.3**
    ...
    assert result.is_valid is False, "Invalid JSON passed validation"
    assert len(result.errors) > 0, "Invalid JSON has no error messages"
    assert (
        "JSON syntax error" in result.errors[0]
    ), "Error message doesn't mention JSON syntax"
        ...
        if "homeassistant" not in parsed:
            ...
            if result.is_valid:
                assert (
                    ...
                    or "homeassistant" not in yaml_content.lower()
                )
    else:
