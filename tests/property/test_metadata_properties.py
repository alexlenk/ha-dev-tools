    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                # Handle both return types (metadata dict or boolean) - only
                # care whether the write succeeded, metadata is fetched
                # separately below regardless of which shape came back.
                if not isinstance(metadata_or_bool, dict) and not metadata_or_bool:
                ...
                ), "Metadata missing 'content_hash' field"
                ...
                assert isinstance(
                    metadata["modified_at"], str
                ), "modified_at should be a string (ISO 8601)"
                assert isinstance(
                    metadata["content_hash"], str
                ), "content_hash should be a string"
                assert (
                    len(metadata["content_hash"]) == 64
                ), "SHA-256 hash should be 64 hex characters"
                assert all(
                    c in "0123456789abcdef" for c in metadata["content_hash"]
                ), "Hash should be hex"
    ...
    **Validates: Requirements 7.5**
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ), "Non-existent file should have exists=False"
            ...
            assert (
                "modified_at" not in metadata
            ), "Non-existent file should not have modified_at"
            assert (
                "content_hash" not in metadata
            ), "Non-existent file should not have content_hash"
    ...
    **Validates: Requirements 7.4**
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                metadata["path"] == blacklisted_file
            ), "Path should match requested file"
            assert (
                metadata["exists"] is False
            ), "Blacklisted file should have exists=False"
            assert (
                metadata["accessible"] is False
            ), "Blacklisted file should have accessible=False"
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for filename in individual_metadata.keys():
                ...
                assert (
                    individual["exists"] == batch["exists"]
                ), f"Exists mismatch for {filename}"
                assert (
                    individual["accessible"] == batch["accessible"]
                ), f"Accessible mismatch for {filename}"

                # If file exists, compare detailed metadata
                if individual["exists"] and batch["exists"]:
                    assert (
                        individual["size"] == batch["size"]
                    ), f"Size mismatch for {filename}"
                    assert (
                        individual["content_hash"] == batch["content_hash"]
                    ), f"Hash mismatch for {filename}"
                    assert (
                        individual["modified_at"] == batch["modified_at"]
