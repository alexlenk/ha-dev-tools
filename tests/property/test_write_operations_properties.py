...
invalid_yaml_content = st.sampled_from(
    [
        "key: value\n  [invalid: yaml: syntax",  # ScannerError - brackets
        "key: value\n\t\ttabs: not allowed",  # ScannerError - tabs disallowed
        "{{unclosed template",  # ParserError - unclosed template
        "key: !!python/object:os.system",  # ConstructorError - dangerous tag
        "key: value\n  invalid: [unclosed",  # ScannerError - unclosed bracket
        "key: value\n    nested:\n  wrong_indent: value",  # ScannerError - indent
...
valid_yaml_filenames = st.builds(
    ...
    ).filter(lambda x: not x.startswith(".") and x not in DEFAULT_BLACKLIST),
)
...
def test_upload_round_trip_property(filename, content):
    ...
    For any valid YAML content, upload then read back should give equivalent content.
    ...
    **Validates: Requirements 5.2**
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Configure security manager to allow read-write access for temp dir
            security_config = {
            ...
            try:
                ...
                metadata = await file_manager.write_file(
                    filename, content, validate_before_write=True
                )
                ...
                # Write failed due to security/validation - acceptable
...
def test_pre_upload_validation_property(filename, content):
    ...
    For any invalid YAML, syntax validation should occur before the file is written.
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert any(
                    ...
                    for keyword in ["validation", "invalid", "yaml", "syntax"]
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                ), "File should not have been modified after hash conflict"
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    len(temp_files) == 0
                ), f"Temporary files should be cleaned up: {temp_files}"
