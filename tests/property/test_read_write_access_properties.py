...
from custom_components.ha_dev_tools.const import OPERATION_READ, OPERATION_WRITE
...
@given(read_only_paths=st.lists(valid_path_string(), min_size=1, max_size=10))
@settings(
...
def test_property_20_read_only_path_enforcement(read_only_paths):
    ...
    **Validates: Requirements 11.1, 11.6**
    ...
    for path in read_only_paths:
        ...
        assert (
            is_valid_write is False
        ), f"Write operation on read-only path '{path}' was incorrectly allowed"
        assert (
            error_message_write is not None
        ), "Denied write operation should have an error message"


@given(write_paths=st.lists(valid_path_string(), min_size=1, max_size=10))
@settings(
    suppress_health_check=[HealthCheck.function_scoped_fixture],
    max_examples=100,
    deadline=5000,
...
def test_property_21_write_path_permissions(write_paths):
    ...
    **Validates: Requirements 11.2**
    ...
    for path in write_paths:
        ...
        assert (
            is_valid_write is True
        ), f"Write operation on write-enabled path '{path}' was incorrectly denied"


@given(common_paths=st.lists(valid_path_string(), min_size=1, max_size=10))
@settings(
    suppress_health_check=[HealthCheck.function_scoped_fixture],
    max_examples=100,
    deadline=5000,
...
def test_property_22_write_path_precedence(common_paths):
    ...
    **Validates: Requirements 11.3**
    ...
    for path in common_paths:
        ...
        assert (
            is_valid_write is True
        ), f"Write on path '{path}' in both read_paths and write_paths was incorrectly denied"
...
def test_property_23_denied_path_supremacy(common_paths):
    ...
    **Validates: Requirements 11.4**
    ...
    for path in common_paths:
        ...
        assert (
            is_valid_read is False
        ), f"Read operation on denied path '{path}' was incorrectly allowed"
        assert (
            error_message_read is not None
        ), "Denied read operation should have an error message"
