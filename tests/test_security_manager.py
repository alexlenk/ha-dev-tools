...
def test_validate_user_permissions_admin(
    hass: HomeAssistant, security_manager, admin_user
):
...
def test_validate_user_permissions_regular_user(
    hass: HomeAssistant, security_manager, regular_user
):
...
def test_allowed_extensions(hass: HomeAssistant, security_manager):
    ...
    # test.py and test.jinja2 are not in the /config/*.{yaml,yml,json,txt}
    # pattern, so they will be rejected in strict allowlist mode even
    # though .py is an allowed extension
...
def test_invalid_configuration_denied_path_traversal(hass: HomeAssistant, caplog):
    """Test handling of denied_paths entries with path traversal (..) (Task 9.3)."""
    import logging

    config = {
        "denied_paths": [
            "/config/custom_blocked.yaml",  # Valid
    ...
    # Valid path should be in denylist
    assert "/config/custom_blocked.yaml" in manager.denylist

    # Should log a warning for the invalid path
    assert "Invalid path in denied_paths" in caplog.text
    assert "path traversal" in caplog.text.lower()


def test_invalid_configuration_denied_path_absolute(hass: HomeAssistant, caplog):
    """Test handling of denied_paths entries outside /config/ or /addon_configs/ (Task 9.3)."""
    import logging
...
def test_invalid_configuration_fallback_to_defaults(hass: HomeAssistant):
    """Test that system still initializes with an invalid denied_paths entry (Task 9.3)."""
    config = {
        "denied_paths": [
            "../etc/passwd",  # Invalid
        ]
...
def test_allowlist_mode_default_behavior(hass: HomeAssistant, caplog):
    """Test default behavior with no read_paths/write_paths specified (Task 9.4)."""
    import logging

    config = {
        # No read_paths or write_paths specified
    }
    ...
    # System always operates in strict allowlist mode
    # Should fall back to the default read-only and write paths
    assert len(manager.allowlist) > 0

    # Should log that it's using default read-only paths
    assert "default read-only paths" in caplog.text.lower()

    # Should have the default paths in the allowlist
    from custom_components.ha_dev_tools.const import DEFAULT_READ_ONLY_PATHS

    for pattern in DEFAULT_READ_ONLY_PATHS:
...
def test_allowlist_mode_directory_matching(hass: HomeAssistant):
    ...
    is_valid, error = manager.validate_file_path(
...
def test_path_normalization_with_redundant_separators(hass: HomeAssistant):
    ...
    is_valid1, error1 = manager.validate_file_path(
...
def test_glob_pattern_question_mark_wildcard(hass: HomeAssistant):
    ...
    config = {
        "read_paths": [
            # Matches configuration.yaml (4-char extension after "ya")
            "/config/configuration.ya??",
...
def test_glob_pattern_multiple_wildcards(hass: HomeAssistant):
    ...
    is_valid, error = manager.validate_file_path(
        "configuration.yaml"
    )  # Doesn't end with 'ations'
    assert is_valid is False
    assert error == ERROR_PERMISSION_DENIED

    is_valid, error = manager.validate_file_path(
        "automations.yml"
    )  # Only 3 chars after 'ya', not 4
    assert is_valid is False
    assert error == ERROR_PERMISSION_DENIED
...
def test_all_safe_storage_files_with_recommended_patterns(hass: HomeAssistant):
    ...
    for file in safe_files:
        ...
        assert (
            is_valid is True
        ), f"{file} should be accessible with recommended patterns"
        assert error is None
...
def test_sensitive_files_blocked_even_with_explicit_allowlist(hass: HomeAssistant):
    ...
    assert (
        is_valid is False
    ), ".storage/auth should be blocked despite being in allowlist"
    assert error == ERROR_BLACKLISTED_FILE

    is_valid, error = manager.validate_file_path(".storage/core.config_entries")
    assert (
        is_valid is False
    ), ".storage/core.config_entries should be blocked despite being in allowlist"
    assert error == ERROR_BLACKLISTED_FILE


def test_safe_and_sensitive_storage_separation(hass: HomeAssistant):
    """Test safe files stay accessible while sensitive files are blocked (Task 11.1, 11.2)."""
...
def test_read_operations_on_read_only_paths(hass: HomeAssistant):
    ...
    from custom_components.ha_dev_tools.const import OPERATION_READ
    ...
    for path in read_only_paths:
        ...
        assert (
            is_valid is True
        ), f"Read operation on read-only path {path} should succeed"
        assert error is None
...
def test_write_operations_denied_on_read_only_paths(hass: HomeAssistant):
    ...
    for path in read_only_paths:
        ...
        assert (
            is_valid is False
        ), f"Write operation on read-only path {path} should be denied"
        assert (
            error == ERROR_WRITE_NOT_PERMITTED
        ), f"Expected ERROR_WRITE_NOT_PERMITTED, got {error}"


def test_error_message_for_write_on_read_only_path(hass: HomeAssistant):
    """Test error is ERROR_WRITE_NOT_PERMITTED for write on a read-only path (Task 12.1)."""
    from custom_components.ha_dev_tools.const import (
        ERROR_WRITE_NOT_PERMITTED,
        OPERATION_WRITE,
    )
...
def test_read_operations_on_write_enabled_paths(hass: HomeAssistant):
    ...
    for path in write_enabled_paths:
        ...
        assert (
            is_valid is True
        ), f"Read operation on write-enabled path {path} should succeed"
        assert error is None
...
def test_write_operations_on_write_enabled_paths(hass: HomeAssistant):
    ...
    for path in write_enabled_paths:
        ...
        assert (
            is_valid is True
        ), f"Write operation on write-enabled path {path} should succeed"
        assert error is None
...
def test_write_paths_precedence_over_read_paths(hass: HomeAssistant):
    ...
    assert (
        is_valid is True
    ), "Write operation should succeed when path is in write_paths"
    assert error is None
...
def test_denied_paths_precedence_over_write_paths(hass: HomeAssistant):
    ...
    assert (
        is_valid is False
    ), "Read operation should be denied when path is in denied_paths"
    assert error == ERROR_BLACKLISTED_FILE

    is_valid, error = manager.validate_file_path("secrets.yaml", OPERATION_WRITE)
    assert (
        is_valid is False
    ), "Write operation should be denied when path is in denied_paths"
    assert error == ERROR_BLACKLISTED_FILE
...
def test_default_operation_type_is_read(hass: HomeAssistant):
    ...
    assert (
        is_valid is True
    ), "Default operation should be read, which should succeed on read_paths"
    assert error is None
...
def test_remove_from_allowlist_runtime(hass: HomeAssistant):
    ...
    assert (
        is_valid is False
    ), "Path removed from allowlist should be denied in allowlist mode"
    assert error == ERROR_PERMISSION_DENIED
...
def test_remove_from_denylist_runtime(hass: HomeAssistant):
    ...
    assert (
        is_valid is True
    ), "Path removed from denylist and added to allowlist should be accessible"
    assert error is None
...
def test_remove_from_denylist_default_sensitive_file(hass: HomeAssistant):
    ...
    config = {"read_paths": ["/config/*.yaml"]}  # Add allowlist config
    manager = SecurityManager(hass, config=config)
    ...
    assert (
        is_valid is True
    ), "Removed default sensitive file should be accessible after removal, in allowlist mode"
    assert error is None
...
def test_get_allowlist_returns_copy_not_reference(hass: HomeAssistant):
    ...
    assert (
        "/config/test.yaml" not in allowlist2
    ), "Modifying returned allowlist should not affect original"
    assert (
        "/config/test.yaml" not in manager.allowlist
    ), "Modifying returned allowlist should not affect original"
...
def test_get_denylist_returns_copy_not_reference(hass: HomeAssistant):
    ...
    assert (
        "/config/test.yaml" not in denylist2
    ), "Modifying returned denylist should not affect original"
    assert (
        "/config/test.yaml" not in manager.denylist
    ), "Modifying returned denylist should not affect original"
...
def test_query_methods_reflect_runtime_modifications(hass: HomeAssistant):
    ...
    assert (
        "/config/test1.yaml" in allowlist
    ), "get_allowlist should reflect runtime additions"
    assert (
        "/config/test2.yaml" in denylist
    ), "get_denylist should reflect runtime additions"
    ...
    assert (
        "/config/test1.yaml" not in allowlist
    ), "get_allowlist should reflect runtime removals"
    assert (
        "/config/test2.yaml" not in denylist
    ), "get_denylist should reflect runtime removals"
